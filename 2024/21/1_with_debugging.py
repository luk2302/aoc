...
def get_paths(from_p, to_p, locations):
    ...
    if len(locations) == 11:
        ...
        if to_p[0] == 0 and from_p[1] == 3:
            return [ypath + xpath]
    invalid = any((from_p[0] + x * dx,from_p[1] + y * dy) == {5:(0,0), 11:(0,3)}[len(locations)]  for x in range(abs(dx) + 1) for y in range(abs(dy) + 1))
...
def solve(d: list[str]):
    ...
    for r in range(0, lc, 1):
        ...
        for path in paths:
            ...
            loc = num_locations["A"] # dir_locations["A"]
            ds = {">": (1,0), "<": (-1,0), "^":(0,-1), "v": (0,1)}
            pressed = ""
            for p in path:
                if p in ds:
                    loc = (loc[0] + ds[p][0], loc[1] + ds[p][1])
                    if loc == (0,3): # (0,0)
                        print("WRONG")
                        pass
                else:
                    pressed += next(i for i, l in num_locations.items() if l == loc) # dir_locations.items()

            if d[r] != pressed:
                print("WRONG", d[r], pressed, path)
                pass
